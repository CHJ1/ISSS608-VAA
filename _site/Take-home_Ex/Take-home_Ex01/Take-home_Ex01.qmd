...
data <- data %>%
  mutate(
    Age_num = suppressWarnings(as.numeric(Age)),  # Convert Age to numeric (suppress NA warning from "90_and_Over")
    AgeGroup = case_when(
...
# Summarize population by age group and sex
summarized_data <- data %>%
  group_by(AgeGroup, Sex) %>%
  summarise(total_pop = sum(Pop)) %>%
  ungroup()

# Add total population for each AgeGroup (across both sexes)
...
# Set AgeGroup as an ordered factor to ensure ascending order on the x-axis
...
# Add annotation using plotly's native function
p_plotly <- p_plotly %>%
  layout(
    height = 450,
    annotations = list(
